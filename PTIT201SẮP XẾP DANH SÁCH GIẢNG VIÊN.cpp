...
public:
	...
	friend void xuly(GiangVien gv[], int n);
	friend bool check_name(GiangVien a, GiangVien b);
};
bool check_name(GiangVien a, GiangVien b)
{
	return a.tr < b.tr;
}
void xuly(GiangVien gv[], int n)
{
	sort(gv, gv + n, check_name);
	for (int i = 0; i < n; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
